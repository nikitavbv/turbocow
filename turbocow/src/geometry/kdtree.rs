...
impl KDTree {
    ...
    fn calculate_sah(bounding_box: &BoundingBox, triangles: Vec<Triangle>, split_axis: u8, v: f64) -> f64 {
        let nodes = KDTree::split_box_by_plane(bounding_box, triangles.clone(), split_axis, v);
        if nodes.len() == 0 {
            return bounding_box.sqare() * triangles.len() as f64;
        }
        let mut sah_value = 0.0;
        for node in nodes {
            match node {
                NodeType::LeafNode(bounding_box, triangles) => sah_value += bounding_box.sqare() * triangles.len() as f64,
                NodeType::Node(_, _) => {},
            };
        }
        sah_value
    ...
    fn split_box_sah(bounding_box: &BoundingBox, triangles: Vec<Triangle>, split_axis: u8) -> Vec<NodeType> {
        ...
        let step = (max - min) / 32.0;
        let mut i = min + step;
        let mut sah_value = KDTree::calculate_sah(bounding_box, triangles.clone(), split_axis, i);
        let mut split_coordinate = i;
        i += step;
        while i < max {
            let tmp_sah = KDTree::calculate_sah(bounding_box, triangles.clone(), split_axis, i);
            if tmp_sah < sah_value {
                sah_value = tmp_sah;
                split_coordinate = i;
            }
            i += step;
        }
        KDTree::split_box_by_plane(bounding_box, triangles, split_axis, split_coordinate)
    }

    fn split_box_by_plane(bounding_box: &BoundingBox, triangles: Vec<Triangle>, split_axis: u8, v: f64) -> Vec<NodeType> {
        let get_coorginate = KDTree::coordinate_fn(split_axis);
        let min = get_coorginate(&bounding_box.first);
        let max = get_coorginate(&bounding_box.second);
    ...
    fn split_box(bounding_box: &BoundingBox, triangles: Vec<Triangle>, split_axis: u8) -> Vec<NodeType> {
        ...
        KDTree::split_box_by_plane(bounding_box, triangles, split_axis, v)
    }
